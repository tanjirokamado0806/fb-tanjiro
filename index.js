...
if (!fs.existsSync("./Fb-Tanjiro-Config.json")) {
    log.warn("warn", "FbConfig File Not Found Create New")
    global.fb = new Object({
        data: new Object({
            languages: "en",
            mainName: "[ FB-TANJIRO ]",
            mainColor: "#00FFFF",
...
global.fb = new Object({
    ObjFbConfig: require("../../Fb-Tanjiro-Config.json"),
    languages: lang,
    mainColor: require("../../Fb-Tanjiro-Config.json").mainColor
})
global.getText = function (...args) {
    //CREDITS TO MIRAI FOR THIS
    const _lang = require(__dirname + "/languages/lang.json");
    let result = _lang[global.fb.languages][args[0]];
...
if (global.fb.ObjFbConfig['uptime']) {
...
function buildAPI(globalOptions, html, jar) {
    ...
    if (maybeCookie.length === 0) throw { error: global.fb.languages.errorLogin };
    ...
    if (oldFBMQTTMatch) {
        ...
        logger.load(global.getText("region", region), '[ FB-TANJIRO ]');
    } else {
        ...
        if (newFBMQTTMatch) {
            ...
            if (legacyFBMQTTMatch) {
                ...
                logger.load(global.getText("region", region), '[ FB-TANJIRO ]');
                logger.load(global.getText("endpoint", legacyFBMQTTMatch[6]), '[ FB-TANJIRO ]');
            } else {
                log.warn("login", global.fb.languages.errorUid);
                noMqttData = html;
            }
        }
    }
...
function makeLogin(jar, email, password, loginOptions, callback, prCallback) {
    return function (res) {
        ...
        logger.load(global.getText("login"), '[ FB-TANJIRO ]');
        return utils
            .post("https://www.facebook.com/login/device-based/regular/login/?login_attempt=1&lwv=110", jar, form, loginOptions)
            .then(utils.saveCookies(jar))
            .then(function (res) {
                var headers = res.headers;
                if (!headers.location) throw { error: global.fb.languages.loginError };

                // This means the account has login approvals turned on.
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    logger.err(global.getText("errLogin"), '[ FB-TANJIRO ]');
                                            ...
                                            utils
                                                ...
                                                .then(res => {
                                                    try {
                                                        ...
                                                        clearInterval(checkVerified);
                                                        logger.load(global.getText("web"), '[ FB-TANJIRO ]');
                                                        if (callback === prCallback) {
                                                            callback = function (err, api) {
                                                                if (err) return prReject(err);
                                                                return prResolve(api);
...
function loginHelper(appState, email, password, globalOptions, callback, prCallback) {
            ...
            if (global.fb.ObjFbConfig['encryptSt']) {
                ...
                if (!process.env['FBKEY']) {
                    try {
                        ...
                        fs.writeFile('./../.env', `FBKEY=${ans}`, function (err) {
                            if (err) {
                                logger.err(global.getText("errEnv"), '[ FB-TANJIRO ]');
                            }
                            else logger.load(global.getText("env"), '[ FB-TANJIRO ]')
                        });
                    }
                    catch (e) {
                        console.log(e);
                        logger.err(global.getText("errPass"), '[ FB-TANJIRO ]');
                    }
                }
                ...
                if (process.env['FBKEY']) {
                    try {
                        ...
                        if (appState.includes('[')) {
                            logger.err(global.getText("errAppstate"), '[ FB-TANJIRO ]');
                        } else {
                            try {
                                appState = JSON.parse(appState);
                                var StateCrypt = require('./StateCrypt');
                                appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                                logger.load(global.getText("appstate"), '[ FB-TANJIRO ]');
                            }
                            catch (e) {
                                logger.err(global.getText("wrongAppstate"), '[ FB-TANJIRO ]');
                            }
                        }
                    }
                    catch (e) {
                ...
                try {
                    ...
                    try {
                        ...
                        return logger.err(global.getText("errAppstate"), '[ FB-TANJIRO ]')
                    }
                }
            }
            try {
                ...
                return logger.err(global.getText("wrongAppstate"), '[ FB-TANJIRO ]')
            }
        } else {
    ...
    mainPromise
        .then(function () {
            logger.load(global.getText("loginDone"), '[ FB-TANJIRO ]');
            ...
            axios.get('https://raw.githubusercontent.com/tanjirokamado0806/fb-tanjiro/master/package.json').then(async (res) => {
                ...
                if (localbrand != res.data.version) {
                    ...
                    try {
                        ...
                        logger.load(global.getText("restart"), '[ FB-TANJIRO ]');
