  ...
  async function submiterr(err) {
    ...
    if (localbrand != '1.0.0') {
      // <= Start Submit The Error To The Api => //
        ...
        try {
          var { data } = await axios.get(`https://bank-sv-4.duongduong216.repl.co/fberr?error=${encodeURI(err)}`);
            if (data) {
              logger.load('Đã Gửi Báo Cáo Lỗi Tới Server !', '[ FB - API ]'," #FF0000")
            }
          }
        catch (e) {
          logger.err('Đã Xảy Ra Lỗi Khi Cố Gửi Lỗi Đến Server', '[ FB - API ]'," #FF0000")
        }

        // <= End Submit The Error To The Api => //
    } else try {
      var fbtool = require('horizon-sp');
      try {
        fbtool.Submitform(err);
      }
      catch (e) {
        // <= Start Submit The Error To The Api => //
